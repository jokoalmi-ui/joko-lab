...
|| 5.6 | **Portabilidad de Joko Lab — auditoría de acoplamiento a Hermes Agent** — Pendiente futuro, no planificado. Origen: mismo vídeo tutorial de Hermes Agent (Marcos/Emowe). Evaluar qué partes del laboratorio dependen exclusivamente de Hermes y podrían no ser portables a otro agente: SOUL.md (formato propio), llamadas a "hermes config set" en apply-decision.sh, estructura de skills (¿sigue agentskills.io o es propietario?), datos en ~/.hermes/ (memoria, sesiones). No refactorizar ahora — no hay necesidad actual de cambiar de agente. Retomar si se evalúa migrar a otro agente, o para asegurar que docs/decisiones/ sea consultable independientemente del agente que lo gestione. | Baja | Pendiente |
